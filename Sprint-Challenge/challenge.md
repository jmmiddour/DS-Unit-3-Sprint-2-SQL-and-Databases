...
- In the Northwind database, what is the type of relationship between the
  ...
  - The `Employee` table can only be joined to the `EmployeeTerritory` table. Then from there, you can join the `EmployeeTerritory` table to the `Territory` table. The relationship for the `Employee` table to `EmployeeTerritory` table is a one-to-many relationship because the `Id` in the `Employee` table is unique but there are many instances of `EmployeeId` in the `EmployeeTerritory` table. The relationship for the `EmployeeTerritory` table to `Territory` table is a one-to-one relationship because both the `TerritoryId` in the `EmployeeTerritory` table and the `Id` in the `Territory` table only have unique values.

- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?
  - A document store, such as MongoDB, is an appropriate solution when you have a large database (larger then ~10 TB) and it is less structured data that does not require immediate consistancy. A good example for this solution would be a company like Facebook that has a vast number of users and a large amount of data that relate to each of those users. Where a document store would not be an appropriate solution would be if you have a database with less than 10 TB and it needs to have a structured schema that also holds consistency guarantees. A good example of this would be a banking institution that needs the ACID guarantee for their multiple transactions that go on every second of every day.

- What is "NewSQL", and what is it trying to achieve?
  - NewSQL is a class of RDBMS that are attempting to provide the scalability of the document store systems while maintaining the ACID guarantees of rational database systems. NewSQL wants to provide a system that can be used by financial institutes to better manage online transaction processing workloads. This is being accomplished by using systems that improve performance and scalability by omitting heavyweight recovery or concurrency control.
